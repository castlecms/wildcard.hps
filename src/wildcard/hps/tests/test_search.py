...
class TestQueries(BaseFunctionalTest):
    ...
    def test_field_index_query(self):
        ...
        self.commit()
        self.hpscatalog.connection.indices.flush()
        time.sleep(1)
    ...
    def test_multi_keyword_index_query(self):
        ...
        self.hpscatalog.connection.indices.flush()
        time.sleep(1)
        el_results = self.catalog(object_provides=[self.event_klass, self.document_klass], SearchableText="new content")
    ...
    def test_date_index_query(self):
        ...
        query = {"query": (start, end), "range": "minmax", "SearchableText": "Event"}
    ...
    def test_text_index_query(self):
        ...
        events = []
        for idx in range(5):
            event = createObject(self.portal, "Event", f"event{idx}", title=f"Some Event {idx}")
            events.append(event)
    ...
    def test_path_index_query(self):
        ...
        self.assertEqual(
            len(self.catalog(path={"depth": 0, "query": "/plone/folder0"}, SearchableText="new content")), 1
        )
        self.assertEqual(
            len(self.catalog(path={"depth": 1, "query": "/plone/folder0"}, SearchableText="new content")), 4
        )
        self.assertEqual(
            len(self.catalog(path={"depth": -1, "query": "/plone/folder0"}, SearchableText="new content")), 9
        )
        self.assertEqual(len(self.catalog(path={"depth": 1, "query": "/plone"}, SearchableText="new content")), 1)
    ...
    def test_brains_indexing(self):
        ...
        self.commit()
        el_results = self.catalog(portal_type="Document", sort_on="getId", sort_order="asc")
...
if HAS_ATCONTENTTYPES:
    from wildcard.hps.testing import WildcardHPS_FUNCTIONAL_TESTING_AT
    ...
    class TestQueriesAT(TestQueries):
        layer = WildcardHPS_FUNCTIONAL_TESTING_AT
