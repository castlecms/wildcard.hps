...
class BaseIndex:
    ...
    def get_value(self, object):  # noqa: A002
        value = None
        attrs = self.index.getIndexSourceNames()
        attr = attrs[0] if len(attrs) > 0 else ""
...
class EDateIndex(BaseIndex):
    ...
    def get_query(self, name, value):
        ...
        if query is None:
            return
        if range_ is None and isinstance(query, (list, tuple)):
            range_ = "min"
        ...
        elif range_ in ("min:max", "minmax") and (type(query) in (list, tuple)) and len(query) == 2:
...
class EZCTextIndex(BaseIndex):
    ...
    def get_query(self, name, value):
        ...
        if name in ("Title", "SearchableText"):
            ...
            queries.append({"match_phrase_prefix": {"Title": {"query": clean_value, "boost": 2}}})
        if name != "Title":
...
class EExtendedPathIndex(BaseIndex):
    ...
    def get_value(self, object):  # noqa: A002
        ...
        if path is not None:
            ...
            if not isinstance(path, (str, tuple)):
                raise TypeError(f"path value must be string or tuple of strings: ({index}), {path})")
        else:
            try:
    ...
    def get_query(self, name, value):
        ...
        for path in paths:
            ...
            filters = []
            if depth == 0:
                andfilters.append({"bool": {"filter": {"term": {name + ".path": path}}}})
                continue
            else:
                filters = [{"prefix": {name + ".path": path}}, {"range": {name + ".depth": {gtcompare: start}}}]
            if depth != -1:
...
class EDateRangeIndex(BaseIndex):
    def create_mapping(self, name):
        return {
            "properties": {f"{name}1": {"type": "date", "store": True}, f"{name}2": {"type": "date", "store": True}}
        }
    ...
    def get_query(self, name, value):
        ...
        date = value.ISO8601()
        return [
            {"range": {f"{name}.{name}1": {"lte": date}}},
            {"range": {f"{name}.{name}2": {"gte": date}}},
        ]
